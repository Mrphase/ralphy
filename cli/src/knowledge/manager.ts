...
function collectEntryLearnings(entry: string): string[] {
	...
	for (const line of lines) {
		...
		if (inLearningsSection && (line.startsWith("- Issues Encountered:") || line.startsWith("## ["))) {
			break;
		}

		if (inLearningsSection && (line.startsWith("  - ") || line.startsWith("- "))) {
			const learning = line.startsWith("  - ") ? line.slice(4).trim() : line.slice(2).trim();
			if (learning) {
